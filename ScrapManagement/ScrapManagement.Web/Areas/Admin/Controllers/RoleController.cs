    ...
    public class RoleController : BaseController<RoleController>
    {
        ...
        public async Task<IActionResult> OnGetCreate(string id)
        {
            ...
            else
            {
                ...
                if (role == null) _notify.Error("Beklenmeyen hata. Rol bulunamadı!");
        ...
        {
            if (ModelState.IsValid && role.Name != "Admin" && role.Name != "User")
            {
                if (string.IsNullOrEmpty(role.Id))
                {
                    await _roleManager.CreateAsync(new IdentityRole(role.Name));
                    _notify.Success("Rol Oluşturuldu");
                }
                else
                {
                    ...
                    _notify.Success("Rol Güncellendi");
                }
        ...
        public async Task<JsonResult> OnPostDelete(string id)
        {
            ...
            if (existingRole.Name != "Admin" && existingRole.Name != "User")
            {
                ...
                if (roleIsNotUsed)
                {
                    ...
                    _notify.Success($"{existingRole.Name} rolü silindi.");
                }
                else
                {
                    _notify.Error("Rol başka bir Kullanıcı tarafından kullanılıyor. Silinemiyor.");
                }
            }
            else
            {
                _notify.Error($"{existingRole.Name} rolünün silinmesine izin verilmiyor.");
            }
            var roles = await _roleManager.Roles.ToListAsync();
            var mappedRoles = _mapper.Map<IEnumerable<RoleViewModel>>(roles);
            var html = await _viewRenderer.RenderViewToStringAsync("_ViewAll", mappedRoles);
